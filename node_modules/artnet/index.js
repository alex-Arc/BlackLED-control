      ...
      ArtnetPacket.parse(msg).then(function(m){
       ...
       //   console.log(m);
          ...
          var node = _.find(me.nodes, function(n){
            return _.map(n.MAC, function(k){return k.toString(16)}).join(':') === _.map(m.MAC, function (k) {return k.toString(16)}).join(':');
          })
          // var node = _.find(me.nodes, function(n){
          //   return [_.map(n.MAC, function(k){return k.toString(16)}).join(':'), n.BindIndex] === [_.map(m.MAC, function (k) {return k.toString(16)}).join(':'), m.BindIndex];
          // })
          ...
          node.id = [node.mac, node.BindIndex]
